...
import { intOr } from './user.query';

async function getAllTourists(req: Request, res: Response, next: NextFunction) {
	try {
		const query = req.query.search as string | undefined;
		const limit = intOr(req.query.limit as string, 10);
		const page = intOr(req.query.page as string, 1);
...
async function getAllAccounts(req: Request, res: Response, next: NextFunction) {
	try {
		...
		const limit = intOr(req.query.limit as string, 20);
		const page = intOr(req.query.page as string, 1);
